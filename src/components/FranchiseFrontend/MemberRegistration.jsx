...
const MemberRegistration = ({ franchiseeName }) => {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      formDataToSend.append("franchiseUnder", franchiseeName || "");
      ...
      // Submit to backend
      const response = await axios.post(
        "http://localhost:8000/api/franchise/create-member",
        formDataToSend,
        {
          headers: {
  ...
  const [userData, setUserData] = useState({ name: "", dob: "", mobNo: "" });
  // searchbar
  const searchMember = async (data) => {
    try {
      const response =await axios.post("http://localhost:8000/api/user/user-exists",{
        mobNo:data?.mobNo, dob:data?.dob, name:data?.name
      })
      if (!response.data.status) {
        toast.success("User does not exist!");
        setUserExist(true);
      } else {
        toast.error("User Exist! Please contact to admin");
      }
    } catch (error) {
      console.log(error)
    }
  ...
  return !userExist ? (
    <>
      <h1 className="font-semibold text-lg">Check the user exist or not</h1>
      <div className="w-full flex md:flex-row flex-col justify-start md:justify-start md:items-center items-start gap-8 mt-6">
        <div className="w-full flex flex-col gap-3">
          <label htmlFor="name">Enter Candidate Name</label>
          <input
            type="text"
            placeholder="Enter Name"
            className="w-full p-2 border-1 border-gray-500 rounded-md"
            onChange={(e) =>
        ...
        <div className="w-full flex flex-col gap-3">
          <label htmlFor="DOB">Enter Candidate DOB</label>
          <input
            type="text"
        ...
        <div className="w-full flex flex-col gap-3">
          <label htmlFor="number">Enter Candidate Number</label>
          <input
            type="tel"
            maxLength={10}
            ...
            className="w-full p-2 border-1 border-gray-500 rounded-md"
            onChange={(e) =>
              setUserData((prev) => ({ ...prev, mobNo: e.target.value }))
            }
          />
        </div>
        <button
          className="bg-red-500 px-3 py-2 text-white rounded-md cursor-pointer font-semibold"
          onClick={() => searchMember(userData)}
        >
          Search
        </button>
      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {value && (
                          <>
                            ...
                            <p>
                              Photo shall be updated after franchisee approval.
                            </p>
                          </>
                        )}
                      </div>
            ...
            <>
              <div className="space-y-6 p-4">
                <h3 className="text-lg font-semibold text-[#7d0a0a] mb-4 flex items-center">
                  Additional Information (Fill below info COMPLETLY to get EXTRA PHOTO UPLOAD)
                </h3>
  ...
  render={({ field: { onChange, value, ...field } }) => (
    <div className="space-y-3">
      ...
      <p className="text-red-500 text-sm">
        {!isStep4Complete()?" Upload a clear, recent photo (JPG Max 200kb)":""}
      </p>
      ...
      {!isStep4Complete() && (
        <p className="text-red-600 text-md mt-1">
          Fill above info COMPLETELY to get EXTRA PHOTO UPLOAD
        </p>
      )}
    </div>
  )}
/>
                  ...
                  <p className="border-2 border-red-500 w-1/2 p-4 font-bold">
                    Upload Extra Photos After Subscribing Registration Package
                  </p>
                  <div className="flex">
                    <div className="md:col-span-4">
                      <Controller
