  ...
  const handleView = async (id) => {
    ...
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/distributor/getCurrentUser/${id}`,
        {
          headers: {
                    ...
                    src={
                      singleUser.userPhotoStatus === "Approved"
                        ? singleUser.profilePic
                          ? `http://127.0.0.1:8000/upload/${singleUser.profilePic}`
                          : `http://127.0.0.1:8000/upload/${singleUser.userPhotoOne}`
