  ...
  const sendOtp = async () => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/user/get-otp",
        {
          email: email,
        }
      );
    ...
    <div className="w-full min-h-screen flex justify-center items-center bg-gray-50">
      <div className="w-full max-w-md bg-white shadow-lg rounded-2xl p-8">
        ...
        <div className="mt-8 text-center">
          <p className="font-bold">For Assistance Contact </p>
          <p className="font-semibold text-md">support@manomilan.com</p>
          <p className="font-semibold text-md">+91 8975553294 / +91 8421018451</p>
          <p className="font-semibold text-gray-500 text-sm py-3">10:00 AM to 7:00 PM</p>
        </div>
      </div>
    </div>
  );
