...
const BiodataProfile = ({ userId }) => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (targetId) {
          ...
          response = await axios.get(`http://localhost:8000/api/admin/get-single-user/${targetId}`);
          ...
          response = await axios.get("http://localhost:8000/api/user/getcurrentuser", {
  ...
  src={
    bioDataDetails.profilePic
      ? `http://localhost:8000/upload/${bioDataDetails.profilePic}`
      : bioDataDetails.userPhotoOne
      ? `http://localhost:8000/upload/${bioDataDetails.userPhotoOne}`
