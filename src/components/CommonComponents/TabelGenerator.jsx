...
export default function TableGenerator({ data, activeTab }) {
  ...
  const singleDistributor = async (userId) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/admin/get-single-distributor/${userId}`,
  ...
  const inactiveDistributor=async(dist)=>{
    ...
    if(result){
      try {
        const response = await axios.put("http://127.0.0.1:8000/api/admin/inactivate-distributor",{
  ...
  const getFranchiseUnderDist = async () => {
    if (distId) {
      try {
        const response = await axios.get(
          `http://127.0.0.1:8000/api/admin/get-franchise-under/${distId}`
        );
  ...
  const getPointsLog = async (distributorId = usId) => {
    ...
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/admin/get/pointsLog/${distributorId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (response.data.status) {
  ...
  const addPointsToSinDist = async () => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/admin/give-points-to-distributor",
        addPointsToDist,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      console.log(response.data)
  ...
  <div className="w-[25%] gap-4 space-y-4 flex flex-col items-center">
    <div className="border border-gray-400 w-48 h-48 rounded-md bg-gray-100 flex items-center justify-center text-gray-400">
      <img src={`http://127.0.0.1:8000/upload/${singleDist.distributorPhoto}`} alt="distributor photo" />
    </div>
