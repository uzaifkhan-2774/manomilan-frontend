...
export default function TableGenerator({ data, activeTab }) {
  ...
  const singleFranchise = async (userId) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/admin/get-single-franchise/${userId}`,
        {
          headers: {
  ...
  const getPointsLog = async (userId) => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/admin/get/pointsLog/${usId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        <div className="w-full flex items-start justify-center">
          ...
          <div className="w-[20%] space-y-2">
            <div className="h-[150px] w-[150px]">
              <img
                src={`http://127.0.0.1:8000/upload/${singleDist.franchisePhoto}`}
                alt="Franchisee"
              />
            </div>
